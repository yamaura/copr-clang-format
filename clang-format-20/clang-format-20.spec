...
# binary installed as /usr/bin/clang-format-20 (coexists with the distro clang).

Name:           clang-format-20
Version:        20.1.8
Release:        1%{?dist}
Summary:        Standalone, statically linked clang-format from LLVM 20
...
statically linked executable named clang-format-20.
...
%install
install -D -m0755 build/bin/clang-format %{buildroot}%{_bindir}/clang-format-20
strip %{buildroot}%{_bindir}/clang-format-20

%check
# Confirm the produced binary is actually static and runs.
file %{buildroot}%{_bindir}/clang-format-20
%{buildroot}%{_bindir}/clang-format-20 --version

%files
%license llvm/LICENSE.TXT
%{_bindir}/clang-format-20

%changelog
* Sat Jun 06 2026 Yuki Yamaura <yamaura@pezy.co.jp> 20.1.8-1
- Initial package: standalone static clang-format from LLVM 20.1.8
